...
def index():
    ...
    return render_template('index.html', ghiChuNguoiDung=ghiChuNguoiDung,  thoiGianTruoc=thoiGianTruoc, congViecNguoiDung=congViecNguoiDung)
...
def doi_mat_khau():
    if request.method == 'POST':
        ...
        if check_password_hash(current_user.MatKhau, mat_khau_cu):
            if mat_khau_moi == xac_nhan_mat_khau:
                mat_khau_moi_bam = generate_password_hash(mat_khau_moi, method='sha256')
                cursor.execute("UPDATE NguoiDung SET MatKhau = ? WHERE MaNguoiDung = ?",
                               (mat_khau_moi_bam, current_user.MaNguoiDung))
                connection.commit()
